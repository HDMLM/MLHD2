...
from core.ui.ui_effects import bind_image_hover, load_small_icon
from core.ui.ui_sound import play_button_click, play_button_hover
...
def build_ui(app):
    ...
    try:
        ...
        if os.path.exists(insignia_font_path):
            ...
            except (tk.TclError, RuntimeError, ValueError) as load_e:
    ...
    font_to_use = (
        app.fs_sinclair_font
        if app.fs_sinclair_font is not None
    ...
    except (OSError, tk.TclError, RuntimeError, ValueError) as e:
    ...
    gw_label = ttk.Label(
        top_right_frame, textvariable=gw_date_var, cursor="hand2", width=14, anchor="w", foreground=flair_fg
    )
    ...
    app.profile_picture_combo = ttk.Combobox(
        mission_frame, textvariable=app.profile_picture, state="readonly", width=32
    )
    ...
    profile_preview_frame = ttk.LabelFrame(
        mission_frame,
    ...
    app.preview_label = tk.Label(
        profile_preview_frame, width=120, height=120, borderwidth=0
    )  # Fixed size for square preview
    ...
    planet_preview_frame = ttk.LabelFrame(
        mission_frame,
        labelwidget=ttk.Label(
            mission_frame,
            text="Planet Preview",
    ...
    app.biome_banner_combo = ttk.Combobox(
        banner_header, textvariable=app.banner_type_var, values=banner_options, state="readonly", width=17
    )
    ...
    def update_planet_preview(*args):
        try:
            ...
            biome_map.get(planet_name, "Mars")
    ...
    def update_sector_preview(*args):
        try:
            ...
            enemy_colors.get(enemy_type, "#ffffff")
    ...
    def update_row_images(*args):
        ...
        subfaction_icon_path = (
            app_path("media", "subfactions", f"{subfaction_type_clean}.png") if subfaction_type_clean else None
        )
        ...
        campaign_icon_path = (
            app_path("media", "campaigns", f"{campaign_type_clean}.png") if campaign_type_clean else None
        )
    ...
    try:
        ...
        app.settings_btn_label = tk.Label(
            top_buttons_row, image=app.settings_btn_img_default, borderwidth=0, highlightthickness=0, cursor="hand2"
        )
        app.settings_btn_label.pack(side=tk.LEFT, pady=0, padx=0)

        # Bind hover enter/leave using the ui_effects helper
        bind_image_hover(
            app.settings_btn_label,
            app.settings_btn_img_default,
            app.settings_btn_img_hover,
        ...
        def _open_settings(event=None):
            ...
            try:
                ...
                try:
                    subprocess.Popen(
                        [sys.executable] + settings_module + ["-ML"],
                        env=env,
                    ...
                    subprocess.Popen(
                        [sys.executable, settings_path, "-ML"], env=env, creationflags=creationflags, close_fds=True
                    )
            except Exception as e:
    ...
    try:
        ...
        bind_image_hover(
            app.help_btn_label,
            app.help_btn_img_default,
            app.help_btn_img_hover,
            on_enter=lambda e: (play_button_hover(), None),
            on_leave=lambda e: None,
        )
    ...
    try:
        ...
        app.invite_btn_label = tk.Label(
            button_icon_frame, image=app.invite_btn_img_default, borderwidth=0, highlightthickness=0, cursor="hand2"
        )
    ...
    enemy_combo = ttk.Combobox(
        details_frame, textvariable=app.enemy_type, values=enemy_types, state="readonly", width=27
    )
    ...
    def build_enemy_missions(enemy):
        ...
        def is_difficulty_dict(value):
            return isinstance(value, dict) and any(isinstance(k, str) and " - " in k for k in value.keys())
    ...
    ttk.Label(details_frame, text="DSS Active:", foreground=flair_fg).grid(
        row=2, column=2, sticky=tk.W, pady=5, padx=(150, 0)
    )
    ...
    def block_excess(event):
        ...
        text = note_entry.get("1.0", "end-1c")
        if len(text) >= MAX_NOTE_CHARS and event.keysym not in (
            "BackSpace",
            "Delete",
            "Left",
            "Right",
    ...
    ratings = [
        "Gallantry Beyond Measure",
        "Outstanding Patriotism",
        "Truly Exceptional Heroism",
        "Superior Valour",
        "Costly Failure",
        "Honourable Duty",
    ...
    try:
        ...
        app.submit_label = tk.Label(
            content, image=app.submit_img_default, borderwidth=0, highlightthickness=0, cursor="hand2"
        )
        ...
        submit_button = ttk.Button(
            content, text="Submit Mission Report", command=app.submit_data, width=130, padding=(0, 30)
        )
    ...
    try:
        ...
        app.export_gui_label = tk.Label(
            export_frame, image=app.export_gui_img_default, borderwidth=0, highlightthickness=0, cursor="hand2"
        )
        ...
        def on_export_gui_click(e):
            ...
            try:
                ...
                subprocess.Popen([sys.executable, "-m", "core.reports.exportGUI"], shell=False)
            except Exception:
                try:
                    exportgui_path = app_path("core", "reports", "exportGUI.py")
                except Exception:
                    exportgui_path = os.path.join(os.path.dirname(__file__), "exportGUI.py")
                subprocess.Popen([sys.executable, exportgui_path], shell=False)
    ...
    try:
        ...
        app.export_planet_label = tk.Label(
            export_frame, image=app.export_planet_img_default, borderwidth=0, highlightthickness=0, cursor="hand2"
        )
        ...
        def on_export_planet_click(e):
            play_button_click()
            try:
                subprocess.Popen([sys.executable, "-m", "core.reports.sub"], shell=False)
            except Exception:
                try:
    ...
    try:
        ...
        app.export_faction_img_hover = load_export_faction_img(
            app_path("media", "SyInt", "ExportFactionButtonHover.png")
        )

        app.export_faction_label = tk.Label(
            export_frame, image=app.export_faction_img_default, borderwidth=0, highlightthickness=0, cursor="hand2"
        )
        ...
        def on_export_faction_click(e):
            play_button_click()
            try:
                subprocess.Popen([sys.executable, "-m", "core.reports.faction"], shell=False)
            except Exception:
                try:
                    faction_path = app_path("core", "reports", "faction.py")
                except Exception:
                    faction_path = os.path.join(os.path.dirname(__file__), "faction.py")
                subprocess.Popen([sys.executable, faction_path], shell=False)
    ...
    try:
        ...
        app.export_7days_label = tk.Label(
            export_frame, image=app.export_7days_img_default, borderwidth=0, highlightthickness=0, cursor="hand2"
        )
        ...
        def on_export_7days_click(e):
            play_button_click()
            try:
                subprocess.Popen([sys.executable, "-m", "core.reports.expWeek"], shell=False)
            except Exception:
                try:
                    expweek_path = app_path("core", "reports", "expWeek.py")
                except Exception:
                    expweek_path = os.path.join(os.path.dirname(__file__), "expWeek.py")
                subprocess.Popen([sys.executable, expweek_path], shell=False)
    ...
    try:
        ...
        app.export_favourites_img_default = load_export_favourites_img(
            app_path("media", "SyInt", "ExportFavouritesButton.png")
        )
        app.export_favourites_img_hover = load_export_favourites_img(
            app_path("media", "SyInt", "ExportFavouritesButtonHover.png")
        )

        app.export_favourites_label = tk.Label(
            export_frame, image=app.export_favourites_img_default, borderwidth=0, highlightthickness=0, cursor="hand2"
        )
        ...
        def on_export_favourites_click(e):
            play_button_click()
            try:
                subprocess.Popen([sys.executable, "-m", "core.reports.favourites"], shell=False)
            except Exception:
                try:
                    fav_path = app_path("core", "reports", "favourites.py")
                except Exception:
                    fav_path = os.path.join(os.path.dirname(__file__), "favourites.py")
                subprocess.Popen([sys.executable, fav_path], shell=False)
    ...
    try:
        ...
        app.export_achievements_img_default = load_export_achievements_img(
            app_path("media", "SyInt", "ExportAchievementsButton.png")
        )
        app.export_achievements_img_hover = load_export_achievements_img(
            app_path("media", "SyInt", "ExportAchievementsButtonHover.png")
        )

        app.export_achievements_label = tk.Label(
            export_frame, image=app.export_achievements_img_default, borderwidth=0, highlightthickness=0, cursor="hand2"
        )
        ...
        def on_export_achievements_click(e):
            play_button_click()
            try:
                subprocess.Popen([sys.executable, "-m", "core.reports.Achievements"], shell=False)
            except Exception:
                try:
                    achievements_path = app_path("core", "reports", "Achievements.py")
                except Exception:
                    achievements_path = os.path.join(os.path.dirname(__file__), "Achievements.py")
                subprocess.Popen([sys.executable, achievements_path], shell=False)
        ...
        export_button = ttk.Button(
            export_frame,
            text="Export Achievements\n        Data to\n     Webhook",
            command=_launch_achievements,
            padding=(6, 5),
            width=16,
        )
