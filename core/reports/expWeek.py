...
search_mission = (
    recent_df["Mission Type"].mode()[0]
    if "Mission Type" in recent_df.columns and not recent_df["Mission Type"].mode().empty
    else "Unknown"
)
MissionCount = (
    recent_df["Mission Type"].value_counts().iloc[0]
    if "Mission Type" in recent_df.columns and not recent_df["Mission Type"].empty
    else 0
)
search_campaign = (
    recent_df["Mission Category"].mode()[0]
    if "Mission Category" in recent_df.columns and not recent_df["Mission Category"].mode().empty
    else "Unknown"
)
CampaignCount = (
    recent_df["Mission Category"].value_counts().iloc[0]
    if "Mission Category" in recent_df.columns and not recent_df["Mission Category"].empty
    else 0
)
search_faction = (
    recent_df["Enemy Type"].mode()[0]
    if "Enemy Type" in recent_df.columns and not recent_df["Enemy Type"].mode().empty
    else "Unknown"
)
FactionCount = (
    recent_df["Enemy Type"].value_counts().iloc[0]
    if "Enemy Type" in recent_df.columns and not recent_df["Enemy Type"].empty
    else 0
)
search_subfaction = (
    recent_df["Enemy Subfaction"].mode()[0]
    if "Enemy Subfaction" in recent_df.columns and not recent_df["Enemy Subfaction"].mode().empty
    else "Unknown"
)
SubfactionCount = (
    recent_df["Enemy Subfaction"].value_counts().iloc[0]
    if "Enemy Subfaction" in recent_df.columns and not recent_df["Enemy Subfaction"].empty
    else 0
)
search_difficulty = (
    recent_df["Difficulty"].mode()[0]
    if "Difficulty" in recent_df.columns and not recent_df["Difficulty"].mode().empty
    else "Unknown"
)
DifficultyCount = (
    recent_df["Difficulty"].value_counts().iloc[0]
    if "Difficulty" in recent_df.columns and not recent_df["Difficulty"].empty
    else 0
)
search_planet = (
    recent_df["Planet"].mode()[0]
    if "Planet" in recent_df.columns and not recent_df["Planet"].mode().empty
    else "Unknown"
)
PlanetCount = (
    recent_df["Planet"].value_counts().iloc[0] if "Planet" in recent_df.columns and not recent_df["Planet"].empty else 0
)
search_sector = (
    recent_df["Sector"].mode()[0]
    if "Sector" in recent_df.columns and not recent_df["Sector"].mode().empty
    else "Unknown"
)
SectorCount = (
...
total_rating = sum(
    rating_mapping.get(row["Rating"], 0)
    for index, row in recent_df.iterrows()
    if "Rating" in recent_df.columns and row["Rating"] in rating_mapping
)
    ...
    "embeds": [
        {
            ...
            "image": {
                "url": "https://cdn.discordapp.com/attachments/1340508329977446484/1426631105397788834/expWeekly.png?ex=68ebed41&is=68ea9bc1&hm=2cf2c9bed4b40597c509dc756eab867be37f20058b5570fd9fb5c18949ae882d&"
            },
