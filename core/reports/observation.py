    ...
    if df.empty:
        ...
        if randint == 1:
            messagebox.showerror(
                "Export Error", "You're favourite thing is clearly disobeying orders. No missions found in the log."
            )
        else:
            messagebox.showerror(
...
# Always-on badges (appear if applicable to user, regardless of settings)
always_on_order = ["bicon", "ticon", "yearico", "PIco"]
# User-selectable badges (up to 4 can be chosen in settings)
...
try:
    with open(app_path("JSON", "DCord.json"), "r") as _f:
        _dcord_preview = json.load(_f)
        display_pref = _dcord_preview.get("display_badges", None)
...
# Add always-on badges (up to 4, but Custom Icon and Test Icon never both appear so max is really 7)
...
if not current_planet_data.empty:
    ...
    planet_success_percentage = (
        (planet_success_missions / planet_total_missions * 100) if planet_total_missions > 0 else 0
    )
    planet_failure_percentage = (
        (planet_failed_missions / planet_total_missions * 100) if planet_total_missions > 0 else 0
    )
    ...
    current_planet_sector = (
        current_planet_data["Sector"].iloc[0]
        if "Sector" in current_planet_data.columns and not current_planet_data.empty
        else "Unknown"
    )

    # Calculate highest kills in a single mission on current planet
    planet_highest_kills = (
        current_planet_data["Kills"].max()
        if "Kills" in current_planet_data.columns and not current_planet_data.empty
        else 0
    )
    ...
    if "Time" in current_planet_data.columns and not current_planet_data.empty:
        ...
        planet_first_deployment = (
            valid_times.min().strftime("%d-%m-%Y %H:%M:%S") if not valid_times.empty else "No deployments"
        )
        planet_last_deployment = (
            valid_times.max().strftime("%d-%m-%Y %H:%M:%S") if not valid_times.empty else "No deployments"
        )
    else:
    ...
    planet_most_common_mission = (
        current_planet_data["Mission Type"].mode().iloc[0]
        if "Mission Type" in current_planet_data.columns
        and not current_planet_data.empty
        and not current_planet_data["Mission Type"].mode().empty
        else "Unknown"
    )

    # Calculate most common mission category on current planet
    planet_most_common_category = (
        current_planet_data["Mission Category"].mode().iloc[0]
        if "Mission Category" in current_planet_data.columns
        and not current_planet_data.empty
        and not current_planet_data["Mission Category"].mode().empty
        else "Unknown"
    )

    # Calculate most common faction on current planet
    planet_most_common_faction = (
        current_planet_data["Enemy Type"].mode().iloc[0]
        if "Enemy Type" in current_planet_data.columns
        and not current_planet_data.empty
        and not current_planet_data["Enemy Type"].mode().empty
        else "Unknown"
    )

    # Calculate most common subfaction on current planet
    planet_most_common_subfaction = (
        current_planet_data["Enemy Subfaction"].mode().iloc[0]
        if "Enemy Subfaction" in current_planet_data.columns
        and not current_planet_data.empty
        and not current_planet_data["Enemy Subfaction"].mode().empty
        else "Unknown"
    )

    # Calculate most common difficulty on current planet
    planet_most_common_difficulty = (
        current_planet_data["Difficulty"].mode().iloc[0]
        if "Difficulty" in current_planet_data.columns
        and not current_planet_data.empty
        and not current_planet_data["Difficulty"].mode().empty
        else "Unknown"
    )

    # Calculate instance counts for each field on current planet
    planet_mission_count = (
        len(current_planet_data[current_planet_data["Mission Type"] == planet_most_common_mission])
        if planet_most_common_mission != "Unknown"
        else 0
    )
    planet_category_count = (
        len(current_planet_data[current_planet_data["Mission Category"] == planet_most_common_category])
        if planet_most_common_category != "Unknown"
        else 0
    )
    planet_faction_count = (
        len(current_planet_data[current_planet_data["Enemy Type"] == planet_most_common_faction])
        if planet_most_common_faction != "Unknown"
        else 0
    )
    planet_subfaction_count = (
        len(current_planet_data[current_planet_data["Enemy Subfaction"] == planet_most_common_subfaction])
        if planet_most_common_subfaction != "Unknown"
        else 0
    )
    planet_difficulty_count = (
        len(current_planet_data[current_planet_data["Difficulty"] == planet_most_common_difficulty])
        if planet_most_common_difficulty != "Unknown"
        else 0
    )
...
faction_kill_lines = ""
faction_icons = {
    "Automatons": ENEMY_ICONS.get("Automatons", ""),
    "Terminids": ENEMY_ICONS.get("Terminids", ""),
    "Illuminate": ENEMY_ICONS.get("Illuminate", ""),
}
...
if (current_planet_data.empty or planet_total_missions == 0) and current_planet not in [
    "Meridia",
    "Angel's Venture",
    "Moradesh",
    "Ivis",
    ...
    embed_description = (
        ...
        + "**INCOMING MESSAGES FROM SUPER EARTH**\n"
        + "> You have yet to make contact with this planet.\n"
        + "> No tactical data available for analysis.\n"
        + f"> Deploy on **{current_planet}** to begin reconnaissance operations.\n"
        + "> Super Earth High Command awaits your first report, Helldiver.\n\n"
    )
    ...
    embed_description = (
        ...
        + "> Detail from **Operation Enduring Peace** was\n"
        + "> not saved from the planetary implosion.\n"
    ...
    embed_description = (
        ...
        + "> Detail from **Operation Ink & Thunder** was\n"
    ...
    embed_description = (
        ...
        + "**INCOMING MESSAGES FROM SUPER EARTH**\n"
        + f"> We have lost contact with **{current_planet}** {PLANET_ICONS.get(current_planet, '')}.\n"
        + "> Detail from **Operation Black Hole Sun** was\n"
        + f"> not saved during the fracture from the Meridian Singularity {PLANET_ICONS.get('Meridia', '')}.\n"
        + "> Please await further orders from Super Earth High Command, Helldiver.\n\n"
    )
    ...
    embed_description = (
        ...
        + "**INCOMING MESSAGES FROM SUPER EARTH**\n"
        + f"> We have lost contact with **{current_planet}** {PLANET_ICONS.get(current_planet, '')}.\n"
        + "> Detail from **Operation Martyr's Calling** was\n"
        + f"> not saved during the fracture from the Meridian Singularity {PLANET_ICONS.get('Meridia', '')}.\n"
        + "> Please await further orders from Super Earth High Command, Helldiver.\n\n"
    )
    ...
    embed_description = (
        ...
        + "> Detail from **Operation Free Space** was\n"
        + "> not saved from the planetary implosion.\n"
        + "> Please await further orders from Super Earth High Command, Helldiver.\n\n"
    )
else:
...
for webhook_url in webhook_urls:
    # Prepare payload and remove keys with None values (Discord validation is strict)
    ...
    # In DEBUG mode request the webhook with wait=true to get a JSON response for debugging
    send_url = webhook_url + ("?wait=true" if DEBUG else "")

    success, response, err = post_webhook(send_url, json_payload=payload, timeout=20, retries=2)
    if success:
        logging.info(
