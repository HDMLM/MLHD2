...
from core.dynamic_icons import get_dynamic_planet_data
...
def _generate_html_export(df: pd.DataFrame) -> str:
    ...
    def _parse_deploy_times(planet_data: pd.DataFrame) -> tuple[str, str]:
        if planet_data.empty or "Time" not in planet_data.columns:
            return "No deployments", "No deployments"
        times_raw = planet_data["Time"].astype(str).str.strip()
        times_normalized = times_raw.str.replace("/", "-", regex=False)
        times_parsed = pd.to_datetime(times_normalized, format="%d-%m-%Y %H:%M:%S", errors="coerce")
        if times_parsed.isna().all():
            times_parsed = pd.to_datetime(times_normalized, errors="coerce", dayfirst=True)
        ...
        if valid_times.empty:
            return "No deployments", "No deployments"
        return (
            valid_times.min().strftime("%d-%m-%Y %H:%M:%S"),
            valid_times.max().strftime("%d-%m-%Y %H:%M:%S"),
        )
    ...
    def _count_value(series: pd.Series, value: str) -> int:
    ...
    def _get_planet_stats(planet: str) -> dict:
        ...
        failed = (
            len(planet_data[planet_data["Rating"] == "Disgraceful Conduct"])
            if "Rating" in planet_data.columns
            else 0
        )
        success = max(0, deployments - failed)
        success_pct = (success / deployments * 100) if deployments else 0
        failure_pct = (failed / deployments * 100) if deployments else 0
        kills = (
            pd.to_numeric(planet_data["Kills"], errors="coerce").fillna(0).sum()
            if "Kills" in planet_data.columns
            else 0
        )
        deaths = (
            pd.to_numeric(planet_data["Deaths"], errors="coerce").fillna(0).sum()
            if "Deaths" in planet_data.columns
            else 0
        )
        kdr = (kills / deaths) if deaths else kills
        highest_kills = (
            pd.to_numeric(planet_data["Kills"], errors="coerce").fillna(0).max()
            if "Kills" in planet_data.columns and not planet_data.empty
            else 0
        )
        sector = (
            planet_data["Sector"].iloc[0]
            if "Sector" in planet_data.columns and not planet_data.empty
            else "Unknown"
        )
        first_deployment, last_deployment = _parse_deploy_times(planet_data)
        mission = _safe_mode(planet_data["Mission Type"]) if "Mission Type" in planet_data.columns else "Unknown"
        category = (
            _safe_mode(planet_data["Mission Category"]) if "Mission Category" in planet_data.columns else "Unknown"
        )
        faction = _safe_mode(planet_data["Enemy Type"]) if "Enemy Type" in planet_data.columns else "Unknown"
        subfaction = (
            _safe_mode(planet_data["Enemy Subfaction"]) if "Enemy Subfaction" in planet_data.columns else "Unknown"
        )
        difficulty = _safe_mode(planet_data["Difficulty"]) if "Difficulty" in planet_data.columns else "Unknown"
        mission_count = _count_value(planet_data["Mission Type"], mission) if "Mission Type" in planet_data.columns else 0
        category_count = (
            _count_value(planet_data["Mission Category"], category)
            if "Mission Category" in planet_data.columns
            else 0
        )
        faction_count = _count_value(planet_data["Enemy Type"], faction) if "Enemy Type" in planet_data.columns else 0
        subfaction_count = (
            _count_value(planet_data["Enemy Subfaction"], subfaction)
            if "Enemy Subfaction" in planet_data.columns
            else 0
        )
        difficulty_count = (
            _count_value(planet_data["Difficulty"], difficulty)
            if "Difficulty" in planet_data.columns
            else 0
        )
        ...
        return {
            ...
            "deaths": float(deaths),
            "kdr": float(kdr),
            "highest_kills": int(highest_kills),
            "sector": str(sector),
            "first_deployment": str(first_deployment),
            "last_deployment": str(last_deployment),
            "mission": str(mission),
            ...
            "subfaction_count": int(subfaction_count),
            "difficulty_count": int(difficulty_count),
            "planet_index": int(planet_index),
        }

    def _campaign_banner_path(campaign: str) -> str:
    ...
    milestone_entries = [
        ...
        ("1000th Ingress", dynamic_planets.get("ingress_1k", "")),
        ("Favourite Planet", dynamic_planets.get("favourite_planet", "")),
    ...
    milestone_cards = [
        ...
        for label, planet in milestone_entries
        if planet and str(planet).strip()
    ]
    dynamic_planet_cards = "".join(milestone_cards)
    if not dynamic_planet_cards:
        dynamic_planet_cards = (
            "<div class=\"milestone-card\">"
    ...
    total_deaths = int(pd.to_numeric(df.get("Deaths", pd.Series(dtype=float)), errors="coerce").fillna(0).sum())
    total_kdr = (total_kills / total_deaths) if total_deaths else total_kills
    failed_missions = len(df[df["Rating"] == "Disgraceful Conduct"]) if "Rating" in df.columns else 0
    success_missions = max(0, total_deployments - failed_missions)
    success_rate = (success_missions / total_deployments * 100) if total_deployments else 0
    failure_rate = (failed_missions / total_deployments * 100) if total_deployments else 0
    major_order_count = df["Major Order"].astype(int).sum() if "Major Order" in df.columns else 0

    most_common_planet = search_planet if "Planet" in df.columns else "Unknown"
    most_common_sector = search_sector if "Sector" in df.columns else "Unknown"
    most_common_campaign = search_campaign if "Mission Category" in df.columns else "Unknown"
    most_common_subfaction = search_subfaction if "Enemy Subfaction" in df.columns else "Unknown"
    most_common_mission = search_mission if "Mission Type" in df.columns else "Unknown"
    ...
    discord_uid = ""
    try:
        discord_uid = str(discord_data.get("discord_uid", ""))
    except Exception:
        discord_uid = ""
    ...
    rendered = (
        ...
        .replace("{{HELLDIVER_TITLE}}", _safe_text(helldiver_title))
        .replace("{{PROFILE_SUMMARY}}", _safe_text(profile_summary))
        ...
        .replace("{{TOTAL_DEATHS}}", _safe_text(total_deaths))
