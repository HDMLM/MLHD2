...
def generate_helldiver_banner(
    ...
    mission_df_snapshot: Optional[pd.DataFrame] = None

    def _read_mission_df_snapshot() -> Optional[pd.DataFrame]:
        nonlocal mission_df_snapshot
        if mission_df_snapshot is not None:
            return mission_df_snapshot.copy()
        try:
            ...
            if not xlsx_path:
                return None
            mission_df_snapshot = get_mission_data_service().read_mission_log(xlsx_path, use_cache=True)
            return mission_df_snapshot.copy()
        except Exception:
            return None
    ...
    def _draw_camouflaged_qr(
        img_rgba: Image.Image, data: str, box: Tuple[int, int, int, int], opacity: int = 70
    ) -> None:
        ...
        try:
            ...
            int(sum(crop.getdata()) / (crop.width * crop.height)) if crop.width and crop.height else 128
        except Exception:
            pass
        ...
        try:
            ...
            if df is not None and not df.empty:
                ...
                if planet_col is not None:
                    ...
                    planets_raw = (
                        df.get("Planet", []).tolist()
                        if hasattr(df.get("Planet", []), "tolist")
                        else list(df.get("Planet", []))
                    )
                    ...
                    if profile_side_val and deco.width > 0:
                        ...
                        try:
                            ...
                            ovd.rounded_rectangle(
                                [(bg_left, bg_top), (bg_left + bg_w, bg_top + bg_h)], radius=8, fill=(25, 25, 25, 140)
                            )
                        ...
                        if target_h > 0 and deco.height > 0:
                            ...
                            try:
                                ...
                                ovd = ImageDraw.Draw(ov)
                                ovd.rounded_rectangle(
                                    [(bg_left, bg_top), (bg_left + bg_w, bg_top + bg_h)],
                                    radius=8,
                                    fill=(25, 25, 25, 140),
                                )
                                banner.alpha_composite(ov)
                        ...
                        if s_num.notna().any():
                            ...
                            try:
                                if maxv > 1e12:  # likely milliseconds
                                    dt = pd.to_datetime(s_num, unit="ms", errors="coerce", utc=True).dt.tz_localize(
                                        None
                                    )
    ...
    if since:
        draw.text((text_x, name_y + 106), f"Logging Since: {since}", fill=(190, 190, 190, 210), font=font_small)
        ...
        for im in icons:
            if im is not None:
                ...
                ph_draw.rounded_rectangle(
                    [(1, 1), (icon - 2, icon - 2)], radius=4, outline=(180, 180, 180, 180), width=1
                )
                # diagonal hint
